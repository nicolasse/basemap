---
description: Design and implement a new feature from scratch — product definition, engineering design, implementation, and validation.
---
...
- The feature name
- A task description covering all use cases from `product.md`
- The `product.md` and `engineering.md` as context (no `implementation.md` yet — it doesn't exist)

If the feature is large, break it into multiple invocations — one per use case or logical chunk. After each chunk, review before continuing.

### 6. Validate

Invoke the `contract-reviewer` agent with the feature and the full diff.

- If violations: fix them before proceeding
- If warnings: report to the user, let them decide
